  ...
  <tbody>
    {Object.values(Breakpoints).map((breakpointName, index) => {
      ...
      const maxWidth = !isEmpty(nextBreakpointName)
        ? `${parseInt(DesignTokens[`SizeBreakpoint${pascalCaseNextBreakpointName}`], 10) - 1}px`
        : '∞'
      ...
      <tr key={breakpointName}>
        ...
        <td>{maxWidth}</td>
        <td>
          <code>{DesignTokens[`MediaQueryFrom${pascalCaseBreakpointName}`]}</code>
          <br />
          <code>{DesignTokens[`MediaQueryTo${pascalCaseNextBreakpointName}`]}</code>
        </td>
      </tr>
    )})}
